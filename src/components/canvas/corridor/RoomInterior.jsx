...
const SUBTITLES = {
    ...
    'PROJECTS': 'Explore my creative projects',
    'THE STUDIO': 'Watch behind the scenes',
    'TECHNICAL SKILLS': 'Explore my technical skills',
...
const RoomInterior = memo(({ label, showRoom, onReady, isExiting }) => {
    ...
    const isGallery = label === 'THE GALLERY' || label === 'PROJECTS';

    // Trigger onReady for generic rooms (which don't have their own component to do it)
    useEffect(() => {
        if (showRoom && !['THE GALLERY', 'PROJECTS', 'THE STUDIO', 'TECHNICAL SKILLS', 'THE ABOUT', "LET'S CONNECT"].includes(label)) {
            onReady?.();
        }
