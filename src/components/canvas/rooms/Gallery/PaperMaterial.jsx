...
const PaperMaterial = forwardRef(({ color = '#e0e0e0', roughness = 0.6, map, side = THREE.DoubleSide, paintProgress, roomOrigin, ...props }, ref) => {
    ...
    const onBeforeCompile = useMemo(() => (shader) => {
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            float r = 0.2;
            float nr = 0.15;
            vec2 p = vec2((vMapUv.x - 0.5) * w, (vMapUv.y - 0.5) * h);
            
            // Corner check
            vec2 dCorner = abs(p) - vec2(w/2.0 - r, h/2.0 - r);
            if (dCorner.x > 0.0 && dCorner.y > 0.0) {
                if (dot(dCorner, dCorner) > r * r) {
                    discard;
                }
            }
            
            // Notch check
            float dxNotch = abs(p.x) - w/2.0;
            float dyNotch = p.y;
            if (dxNotch * dxNotch + dyNotch * dyNotch < nr * nr) {
                discard;
            }
            `
        );
